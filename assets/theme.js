  ...
  var ProductVariants = /*#__PURE__*/function () {
    ...
    _createClass(ProductVariants, [{
      ...
      value: function _updateSku(newVariant) {
        var productSkuNumber = document.querySelector('.ProductMeta__SkuNumber');
        
        if (!productSkuNumber || !newVariant) {
          return;
        }
      ...
      value: function _updateAddToCartButton(newVariant) {
        var addToCartButton = this.element.querySelector('.ProductForm__AddToCart'),
            isPreOrder = addToCartButton.classList.contains("preorder_atc"),
        ...
        if (!newVariant) {
          ...
          if (newVariant['available'] || isPreOrder) {
            ...
            var atc_text = isPreOrder ? window.languages.productFormAddToCartPreorder : window.languages.productFormAddToCart;
            if (undefined === this.options['showPriceInButton'] || this.options['showPriceInButton']) {
              addToCartButton.innerHTML = "\n            <span>".concat(atc_text, "</span>\n            <span class=\"Button__SeparatorDot\"></span>\n            <span data-money-convertible>").concat(Currency.formatMoney(newVariant['price'], window.theme.moneyFormat), "</span>\n          ");
            } else {
              addToCartButton.innerHTML = "<span>".concat(atc_text, "</span>");
            }
          } else {
            addToCartButton.setAttribute('disabled', 'disabled');
            addToCartButton.classList.add('Button--secondary');
        ...
        if (Responsive.matchesBreakpoint('pocket') || !this.options['stackProductImages']) {
          for (var i = 0; i !== this.productSlideshow.flickityInstance.cells.length; ++i) {
            ...
            if (mediaId === variant['featured_media']['id']) {
              this.productSlideshow.selectCell(parseInt(cellElement.getAttribute('data-media-position')));
            }
          }
        } else {
      ...
      value: function _switchToImage(event, target) {
        ...
        for (var i = 0; i !== this.productSlideshow.flickityInstance.cells.length; ++i) {
          ...
          if (mediaId === parseInt(target.getAttribute('data-media-id'))) {
            this.productSlideshow.selectCell(parseInt(cellElement.getAttribute('data-media-position')));
            /*this.productSlideshow.selectCell(parseInt(cellElement.getAttribute('data-media-position')) - 1); Removed the -1 as we refactored the slider, index starts from 0 */
          }
        }
      }
      /**
